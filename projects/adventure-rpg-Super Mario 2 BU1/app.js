...
const oneup = new GenerateItem('1 UP', 30, 0)
const mushroom = new GenerateItem('Mushroom', 40, 0)
const starman = new GenerateItem('Starman', 35, 3)
...
function printFullInventory(){
    ...
    console.log(`\nYour current health is ${mainHero.hp} and your attack is ${mainHero.ap}`)
}

console.log('\nWelcome to Subcon, the land of dreams! Our once-beautiful home now suffers at the hands of the evil Wart. Please join us on this quest to defeat him and restore peace to our dream world.!')
console.log(`
...
console.log(`\nThanks for joining us, ${name}! Only you can free us from Wart's evil tyranny.`)

let choice = ask.keyIn(`\nWhich hero would you like to choose: [m] Mario, [l] Luigi, [t] Toad, or [p] Princess Toadstool? `, {limit: 'mltp'})
    if (choice === 'm'){
        mainHero.name = 'Mario';
...
while (isAlive && !hasWon && wartAlive){
    let choice = ask.keyIn(`\n${mainHero.name}, would you like to [w] walk around and explore the magical land of Subcon, [l] look around for helpful items, [i] check inventory, or [q] quit the game? `, {limit: 'wliq'})
    if (choice === 'w'){
        walk() 
    } else if (choice === 'l'){
        lookAround()
...
function lookAround(){
    ...
    if(random === 2){
        ...
        let choice = ask.keyIn(`\nWould you like to [w] walk around and explore the magical land of Subcon, [l] look around for helpful items, [i] check inventory, or [q] quit the game? `, {limit: 'wliq'})
        if (choice === 'w'){
            walk() 
        } else if (choice === 'l'){
            lookAround()
...
`) 
        ...
        let choice = ask.keyIn(`\nYou didn't find any weapons or health items. Would you like to [w] walk around and continue to explore the magical land of Subcon, [l] look around more for helpful items, [i] check inventory, or [q] quit the game? `, {limit: 'wliq'})
        if (choice === 'w'){
            walk() 
        } else if (choice === 'l'){
            lookAround()
...
function findItem(items){
    ...
    if (mainHero.ap > 1){
        console.log(`\nYour attack power has already been increased by a previous item.`)
        let choice = ask.keyIn(`\nWould you like to drop your current item and use the ${foundItem.name} instead? Enter [y] yes or [n] no.`, {limit: 'yn'})
        if (choice === 'n'){
            let choice = ask.keyIn(`\nYou left ${foundItem} behind. Would you like to [w] walk around and continue to explore the magical land of Subcon, [l] look around more for helpful items, [i] check inventory, or [q] quit the game? `, {limit: 'wliq'})        
        } else (choice === 'y')
                if (foundItem === oneup){
                    inventoryItems.pop();        
                    console.log(`\nGreat job ${mainHero.name}, you found a ${foundItem.name}! This brought you ${foundItem.hp} health points and ${foundItem.ap} attack points.`) 
                    ...
                    inventoryItems.pop()
                    console.log(`\nGreat job ${mainHero.name}, you found a ${foundItem.name}! This brought you ${foundItem.hp} health points and ${foundItem.ap} attack points.`) 
                } else (console.log(`\nGreat job ${mainHero.name}, you found a ${foundItem.name}! This has been added to your inventory and brought you ${foundItem.hp} health points and ${foundItem.ap} attack points.`) )
                    mainHero.hp += foundItem.hp
                    mainHero.ap += foundItem.ap
...
function wonItem(items){
    ...
    console.log(`\nYou have been rewarded with a ${wonItem.name} from their stash! This has been added to your inventory and brought you ${wonItem.hp} health points and ${wonItem.ap} attack points.`) 
...
function enemyEncounter(){
    const currEnemy = selectEnemy()
    let choice = ask.keyIn(`\nOh no, you encountered ${currEnemy.name}! ${currEnemy.description} Would you like to [a] attack ${currEnemy.name}, [r] run away from ${currEnemy.name}, or [q] quit the game? `, {limit: 'arq'})
    if(choice === 'a'){
        attack(currEnemy)
    } else if (choice === 'r'){
        run(currEnemy)
...
function attack(currEnemy) {
    ...
    if (mainHero.hp > 0 && currEnemy.hp > 0) {
        console.log(`\nYou attack ${currEnemy.name}, their health is now ${currEnemy.hp}. ${currEnemy.name} struck you and your health is ${mainHero.hp}.`)
        let choice = ask.keyIn(`\nWould you like to [a] attack ${currEnemy.name}, [r] run away from ${currEnemy.name}, [i] check inventory, or [q] quit the game? ` , {limit: 'ariq'})
        if (choice === 'a') {
            attack(currEnemy)
            // mainHero.ap = 1
        } else if (choice === 'r'){
            ...
            isAlive = false 
            console.log(`\nBad news, ${currEnemy.name} has defeated you! You have failed to save Subcon from the evil Wart.`)  
            console.log(`
        ...
        console.log(`\nGreat job, you defeated ${currEnemy.name}!`)
        if (currEnemy.name === "Shyguy"){
            ...
            wartAlive = false
                console.log(`\nThank you, ${mainHero.name}! You have freed us from Wart's tyranny! Now we can wake up from this terrible dream.`)
...
function run(currEnemy) {
    ...
    if (random === 1){
        console.log('\nWow, you are fast! You have successfully escaped your enemy!')
    } else {
        ...
        console.log(`\nYou attempted to escape but ${currEnemy.name} has caught you. Careful, now ${currEnemy.name} is mad and their attack could cause more damage! 
        ${currEnemy.name}'s health is ${currEnemy.hp}. ${currEnemy.name} struck you and your health is ${mainHero.hp}.`)
        let choice = ask.keyIn(`\nWould you like to [a] attack ${currEnemy.name}, [r] run away from ${currEnemy.name}, or [q] quit the game? ` , {limit: 'arq'})
        if (choice === 'a') {
            attack(currEnemy)
        } else if (choice === 'r'){
            run(currEnemy)
